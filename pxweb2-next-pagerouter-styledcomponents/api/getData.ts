...
export const getData = async (id: string, valueCodes: { [key: string]: string[]; }, language: string ) => {
    ...
    const response = await fetch(`https://api.scb.se/OV0104/v2beta/api/v2/tables/${id}/data?lang=${language}&outputFormat=json&${valueCodeQueryStrings.join("&")}`);
